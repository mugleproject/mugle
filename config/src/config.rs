...
pub const SERVER_CONFIG_FILE_NAME: &str = "mugle-server.toml";
const SERVER_LOG_FILE_NAME: &str = "mugle-server.log";
const MUGLE_HOME: &str = ".mugle";
const MUGLE_CHAIN_DIR: &str = "chain_data";
...
fn get_mugle_path(chain_type: &global::ChainTypes) -> Result<PathBuf, ConfigError> {
...
fn check_api_secret_files(
	...
	let mugle_path = get_mugle_path(chain_type)?;
...
impl GlobalConfig {
	...
	pub fn for_chain(chain_type: &global::ChainTypes) -> GlobalConfig {
		...
		match *chain_type {
			...
			global::ChainTypes::Testnet => {
				defaults.api_http_addr = "127.0.0.1:16813".to_owned();
				defaults.p2p_config.port = 16814;
				defaults
					.stratum_mining_config
					.as_mut()
					.unwrap()
					.stratum_server_addr = Some("127.0.0.1:16816".to_owned());
				defaults
					.stratum_mining_config
					.as_mut()
					.unwrap()
					.wallet_listener_url = "http://127.0.0.1:16815".to_owned();
			}
			global::ChainTypes::UserTesting => {
				defaults.api_http_addr = "127.0.0.1:26813".to_owned();
				defaults.p2p_config.port = 26814;
				defaults.p2p_config.seeding_type = p2p::Seeding::None;
				defaults
					.stratum_mining_config
					.as_mut()
					.unwrap()
					.stratum_server_addr = Some("127.0.0.1:26816".to_owned());
				defaults
					.stratum_mining_config
					.as_mut()
					.unwrap()
					.wallet_listener_url = "http://127.0.0.1:26815".to_owned();
			}
	...
	/// Update paths
	pub fn update_paths(&mut self, mugle_home: &PathBuf) {
		// need to update server chain path
		let mut chain_path = mugle_home.clone();
		chain_path.push(MUGLE_CHAIN_DIR);
		self.members.as_mut().unwrap().server.db_root = chain_path.to_str().unwrap().to_owned();
		let mut api_secret_path = mugle_home.clone();
		...
		let mut foreign_api_secret_path = mugle_home.clone();
		...
		let mut log_path = mugle_home.clone();
