...
When you are starting to contribute to mugle, we really would appreciate if you come by the gitter chat channels.

In case of problems with trying out mugle, before starting to contribute, there's the [Support chat](https://gitter.im/mugle_community/support). Write there about what you've done, what you want to do, and maybe paste logs through a text paste webservice.
...
The most important thing you can do alongside - or even before - changing code, is adding tests for how mugle should and should not work. See the various `tests` folders and derive test that are already there in mugle.
...
For example: `fix: a panick on xxx when mugle exiting`. Please don't worry if you can't find a suitable prefix, this's just optional, not mandatory.
...
**Note**: To work with mugle you must use `rustup`. Linux package managers typically carry a too old rust version.
...
3. Please don't ~~`cargo +nightly fmt`~~ because all mugle developers are using stable rustfmt. Also please don't rustfmt files that your code changes does not touch to avoid causing merge conflicts.

## Thanks for any contribution

Even one word correction are welcome! Our objective is to encourage you to get interested in Mugle and contribute in any way possible. Thanks for any help!
