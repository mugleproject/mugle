...
/// Mugle server commands processing
...
use mugle_p2p::msg::PeerAddrs;
use mugle_p2p::PeerAddr;
...
fn start_server_tui(
	...
	if config.run_tui.unwrap_or(false) {
		warn!("Starting MUGLE in UI mode...");
...
/// stopping the Mugle blockchain server. Processes all the command line
/// arguments to build a proper configuration and runs Mugle with that
/// configuration.
pub fn server_command(
	...
	if let Some(a) = server_args {
		match a.subcommand() {
			...
			("", _) => {
				println!("Subcommand required, use 'mugle help server' for details");
			}
			(cmd, _) => {
				println!(":: {:?}", server_args);
				panic!(
					"Unknown server command '{}', use 'mugle help server' for details",
					cmd
				);
			}
		}
