...
use crate::consensus::MUGLE_BASE;
...
pub fn amount_from_hr_string(amount: &str) -> Result<u64, Error> {
	...
	let (mugles, nmugles) = match amount.find('.') {
		None => (parse_mugles(amount)?, 0),
		Some(pos) => {
			let (gs, tail) = amount.split_at(pos);
			(parse_mugles(gs)?, parse_nmugles(&tail[1..])?)
		}
	};
	Ok(mugles * MUGLE_BASE + nmugles)
}

fn parse_mugles(amount: &str) -> Result<u64, Error> {
	if amount == "" {
		Ok(0)
	} else {
		amount
...
lazy_static! {
	static ref WIDTH: usize = (MUGLE_BASE as f64).log(10.0) as usize + 1;
}

fn parse_nmugles(amount: &str) -> Result<u64, Error> {
...
pub fn amount_to_hr_string(amount: u64, truncate: bool) -> String {
	let amount = (amount as f64 / MUGLE_BASE as f64) as f64;
