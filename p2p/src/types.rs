...
impl From<mugle_store::Error> for Error {
	fn from(e: mugle_store::Error) -> Error {
		Error::Store(e)
	}
}
...
impl PeerAddr {
	...
	/// defaults to port 6814 on mainnet and 16814 on testnet.
	pub fn from_ip(addr: IpAddr) -> PeerAddr {
		let port = if global::is_testnet() { 16814 } else { 6814 };
...
/// included in mugle-server.toml, but we don't want them to ever return none
impl P2PConfig {
	/// return ban window
...
pub trait ChainAdapter: Sync + Send {
	...
	/// Get the Mugle specific tmp dir
