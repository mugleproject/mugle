...
impl Default for LoggingConfig {
	fn default() -> LoggingConfig {
		LoggingConfig {
			...
			log_file_path: String::from("mugle.log"),
...
/// This filter is rejecting messages that doesn't start with "mugle"
/// in order to save log space for only Mugle-related records
#[derive(Debug)]
struct MugleFilter;

impl Filter for MugleFilter {
	fn filter(&self, record: &Record<'_>) -> Response {
		if let Some(module_path) = record.module_path() {
			if module_path.starts_with("mugle") {
